...
function SelectNoteButton({note}: Props) {
  ...
  return (
    <SidebarMenuButton asChild className={`items-start gap-0 pr-10 ${note.id === noteId && "bg-sidebar-accent/50"}`}>
        <Link href={`/?noteId=${note.id}`} className="flex h-fit flex-col">
            <p className="w-full overflow-hidden truncate text-muted-foreground text-ellipsis whitespace-nowrap">{noteText}</p>
